...
;; reg-subscribe是注册订阅, 在app-db注册atom路径
;; subscribe订阅, 加上app-db的路径就可以取出来了
...
;;
(reg-sub
  :showing          ;; usage:   (subscribe [:showing]) ;; => #<Reaction 106: :all>
...
;; usage: (subscribe [:sorted-todos]) => #<Reaction 104: {1 {:id 1, ..}} , (type @~) cljs.core/PersistentTreeMap
;; subscribe订阅,相当于 `@todos` 取出数据,提供给ClojureScript函数操作,repl测试等
...
;;
(reg-sub
  :todos        ;; usage:   (subscribe [:todos])  => #<Reaction 105: ({:id 1, :title "dsadasdasdsasdadsa" ...
...
(reg-sub
  :visible-todos   ;; (subscribe [:visible-todos]) => #<Reaction 107: ({:id 1, :title "dsadasdasdsasdadsa"...
...
(reg-sub
  :all-complete? ;; (subscribe [:all-complete?]) ;;=> #<Reaction 108: false>
  :<- [:todos]
  (fn [todos _]
    (every? :done todos)))

(reg-sub
  :completed-count ;; (subscribe [:completed-count]) ;;=> #<Reaction 110: 0>
  :<- [:todos]
  (fn [todos _]
    (count (filter :done todos))))

(reg-sub
  :footer-counts ;; (subscribe [:footer-counts]) ;;=> #<Reaction 109: [8 0]>
  :<- [:todos]
  :<- [:completed-count]
