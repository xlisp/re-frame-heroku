...
;; -- Event Handlers 事件操作者 ----------------------------------------------------------

;; usage:  (dispatch [:initialise-db])
;;
;; This event is dispatched in the app's `main` (core.cljs).
;; * 事件是异步发送给app-db的: `(dispatch-sync [:initialise-db])`
;; It establishes initial application state in `app-db`.
;; That means merging:
;;   1. Any todos stored in LocalStore (from the last session of this app) ;; app-db保存于LocalStore
;;   2. Default initial values ;;默认为初始化的值
;;
...
;; registration.
(reg-event-fx                 ;; part of the re-frame API ;; 可以列出所有的re-frame API对比学习
  :initialise-db              ;; event id being handled  ;; 已操作的事件id
