...
The `home-routes` handler in the `stevechan.routes.home` namespace
...
The `home-page` function will in turn call the `stevechan.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/stevechan/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `stevechan.handler` namespace:
...
Request middleware functions are located under the `stevechan.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `stevechan.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `stevechan.db.core` in some other namespace.
